  ...
  computed: {
    ...
    calcCpuList () {
      ...
      const temp = this.configLimitList.reduce((all, cur) => {
        ...
        (!item.rule.memory ||
          (item.rule.memory.opt === 'eq' ? this.handleRange(item.rule, 'memory') :
      ...
      if (temp.length > 0) {
        ...
        const cpuArr = temp.reduce((all, cur) => {
          if (cur.result.cpu.opt === 'eq') {
            ...
            const result = tempCpu.filter(item => !cur.result.cpu.value.includes(item));
            all.push(result);
          }
          return all;
        }, []);
        cpuOpt = this.handleMixed(...cpuArr);
        if (cpuOpt.length === 0) { // 没有交集的时候取全部
          temCpu = this.cpuList;
        } else {
    ...
    calaMemoryList () {
      ...
      let temp = this.configLimitList.reduce((all, cur) => {
        ...
        return (
          ...
          (!item.rule.cpu ||
            (item.rule.cpu.opt === 'eq' ? item.rule.cpu.value.includes(this.params.cpu) :
      ...
      if (this.memoryList[0]?.type === "radio") {
        ...
        this.memoryList.forEach(item => {
          originmemory_arr.push(...this.createArr([item.min_value, item.max_value]));
        });
      }
      // 最小，最大值求交集
      ...
      let memory_arr = ruleResult.reduce((all, cur) => {
        ...
        if (cur.result.memory.opt === 'eq') {
          all.push(this.createArr([cur.result.memory.min * 1, cur.result.memory.max === '' ? memoryMax : cur.result.memory.max * 1]));
        } else {
          let result = this.createArr([cur.result.memory.min * 1, cur.result.memory.max === '' ? memoryMax : cur.result.memory.max * 1]);
      ...
      if (memoryOpt.length === 0) { // 取交集数据为空的时候
    ...
    calcImageList () {
      ...
      /* 限制只针对自定义，不支持套餐 */
      if (this.activeName === "custom" && this.limitNum && this.configLimitList.length > 0) {
